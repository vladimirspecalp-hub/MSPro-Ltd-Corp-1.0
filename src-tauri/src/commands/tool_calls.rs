...
[
  ...
  {
    "name": "create_post",
    "description": "⚠️ DEPRECATED — для новых исполнителей используй create_org_agent / set_agent_card. Posts — legacy-структура. Создать новый пост в одном из 8 отделений Хаббарда. Используй ТОЛЬКО когда Владелец явно попросил создать или добавить ПОСТ (не агента) — не выдумывай посты сам по контексту разговора.",
    "parameters": {
      "type": "object",
      "properties": {
  ...
  {
    "name": "update_post",
    "description": "⚠️ DEPRECATED — для новых исполнителей используй set_agent_card. Posts — legacy-структура. Изменить существующий пост: переименовать slug, обновить название, ЦКП, метрику, перенести в другое отделение или сменить статус (active/paused/archived).",
    "parameters": {
      "type": "object",
      "properties": {
  ...
  {
    "name": "archive_post",
    "description": "⚠️ DEPRECATED — Posts — legacy-структура. Перевести пост в архив (soft-delete). Пост исчезает с Главной, но вся история статистик и задач сохраняется. Возврат — через update_post со status=active.",
    "parameters": {
      "type": "object",
      "properties": {
        "slug": {"type": "string"}
