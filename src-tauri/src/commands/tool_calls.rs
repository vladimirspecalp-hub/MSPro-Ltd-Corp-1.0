...
2. **Формат.** ОДИН JSON в ОДНОМ блоке `<tool_call>`. Без markdown-fence
   (`````json``), без комментариев внутри JSON, без trailing commas.
   Пример:
   ```
   <tool_call>
...
3. **Где разместить.** Можно сначала текстом объяснить Владельцу что собираешься
   ...
   системным сообщением (Владелец увидит зелёный или красный плашко).
4. **Несколько инструментов в одном ответе.** Выводи несколько `<tool_call>`
   блоков подряд — каждый исполняется атомарно.
5. **Reasoning.** Если хочешь подумать — оборачивай в `<think>...</think>`, эти
   блоки тоже скрываются от Владельца. Не пиши пустых tool_call'ов «для теста».
6. **Slug постов** бери ровно из блока «Текущие Состояния Постов» выше — не
   выдумывай несуществующие slug'и, иначе инструмент вернёт ошибку.
"#;
