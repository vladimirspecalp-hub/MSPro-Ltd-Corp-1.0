...
- **Статус (2026-06-07):**
  - ✅ **Заход 1** (Диспетчер + единый `<FileActions>`): «📄 Открыть файл» + «📂 Открыть папку», Outbox-guard через `safe_artifact_path`. **Cursor APPROVE.** БЕЗ «⬇ Скачать» (директива Владельца: только открыть файл/папку).
  - ✅ **Заход 2** (чат Гендира — код готов → Cursor): миграция 10 `chat_messages.spawned_task_id` + **обязательный** self-healing в `lib.rs::setup` (R-T-006). Backend отдаёт task_id **структурно** (tuple `(ToolExecution, Option<String>)` из `execute_send_to_dispatcher` → INSERT с колонкой + событие `ceo-tool-result`, **не парсинг текста**). Новая команда `list_result_artifacts(task_id)` резолвит parent→child через `dispatcher_logs.parent_task_id`, скобки явные: `(task_id=? OR task_id IN (...)) AND rejected_at IS NULL`. Новый `ChatTaskArtifacts.tsx` под ⚡-сообщением, live по `post-executor-finished`. `#[sqlx(default)]` на поле → старые SELECT'ы не падают FromRow (регресс-тест `chat_message_fromrow_tolerates_missing_and_present_column`). Проверки: **cargo 139/0**, **tsc 0**. Граница: ТОЛЬКО чат Гендира (чат агента — отдельный заход). Без bump (копим к 1.0.36).
...
## BL-P1-020 — open_artifact_folder: строгость + тест (🟡 Cursor, BL-P1-018 follow-up)
- **Описание:** две не-блокер заметки Cursor по `open_artifact_folder` (artifacts.rs). (1) После `abs.parent()` можно ещё раз `canonicalize()` папку и проверить `starts_with(outbox_root)` для пущей строгости (сейчас защита уже есть через `safe_artifact_path` для файла — папка = его родитель, всегда внутри Outbox; это доп. ремень). (2) unit-тест на `open_artifact_folder` (по желанию).
- **Приоритет:** Low (Cursor APPROVE — безопасность уже подтверждена; это усиление).
- **Оценка:** ~15-20 мин обе.
- **Пререквизиты:** нет. (1) — **заодно со следующей правкой** `artifacts.rs` (директива Владельца, не отдельным заходом). (2) — опционально.
